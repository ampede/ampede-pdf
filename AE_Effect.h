...
// [20397] - you can define A_SUPPRESS_A_INTERNAL_WARNING if you have a plugin
// that breaks this rule innocently. 
	...
	#define A_INTERNAL_TEST_TWO 1
		...
		#pragma message("Oh dear, you have included AE_Effect.h once with A_INTERNAL set and once without, bad things are gonna happen")
	#endif
#endif
...
enum {
	...
	PF_OutFlag2_DOESNT_NEED_EMPTY_PIXELS		= 1L << 6,		// PF_Cmd_GLOBAL_SETUP
...
enum {
	...
	PF_ValueDisplayFlag_PERCENT		= 1 << 0,		// append % to value display for float sliders (for fixed-point sliders, also maps range into 0-100%)
...
typedef struct {
	/* PARAMETER VALUE */
	PF_ParamValue	value;		// at PF_ParamFlag_SUPERVISE will contain ID of button pushed;
								// not valid at any other time

	union {
		const char	*namesptr; /* button names, '|' separator */
	} u;
} PF_ButtonDef;
...
typedef struct {
	...
	long			unused;		// Once upon a time was reserved_tdb
	PF_ParamDefUnion u;
